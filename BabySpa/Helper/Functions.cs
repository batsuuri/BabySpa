...
namespace BabySpa
{
    ...
    public struct Result
    {
        ...
        public class Func
    {
        static internal string ResFileName = "LeXPro.Resource.test";
        ...
        public static string GetMACAddress(string RemoteHostName)
        {
            try
            {
                ...
                Func.SendARP((int)Dns.GetHostEntry(RemoteHostName).AddressList[0].Address, 0, pMacAddr, ref length);
        ...
        public static string DecimalFix(object pobj)
        {
            return Func.ToStr(pobj).Replace(",","");
        }
        ...
        {
            long.TryParse(Convert.ToString(hex), NumberStyles.HexNumber, (IFormatProvider)null, out long result);
            return result;
        }
        ...
        public static object JsonToObject(string json,object obj) {
            ...
            try
            {
                tmp = Newtonsoft.Json.JsonConvert.DeserializeObject(json,obj.GetType());
            }
            catch (Exception ex)
            {
        ...
        public static object JsonToObject(object json, object obj)
        {
            return JsonToObject(Func.ToStr(json),obj);
        }
        ...
        #region Number to texting 

        private static string GetStrValue(string S, ResourceManager ReManager, string Lang)
        {
            int N;
            //decimal v;
            ...
            D = "0000000000000000000" + S; //v.ToString();

            for (int i = 1; i <= 5; i++)
            {
                //N = Val(Mid(D, D.Length - i * 3 + 1, 3));
                N = Convert.ToInt32(D.Substring(D.Length - i * 3, 3));

                if (i == 1)
                {
                    if (N > 0)
                    {
                        st = StrV(N, ReManager, Lang);
                    }
                }
                else if (i == 2)
                {
                    if (N > 0)
                    {
                        if (st.Length <= 0)
                        {
                            ...
                            st = StrV(N, ReManager, Lang) + " " + ReManager.GetString("stp_thousand") + " " + st;
                        }
                        else
                        {
                            st = StrV(N, ReManager, Lang) + " " + ReManager.GetString("big_thousand") + " " + st;
                        }
                    }
                }
                else if (i == 3)
                {
                    if (N > 0)
                    {
                        st = StrV(N, ReManager, Lang) + " " + ReManager.GetString("big_million") + " " + st;
                    }
                }
                else if (i == 4)
                {
                    if (N > 0)
                    {
                        st = StrV(N, ReManager, Lang) + " " + ReManager.GetString("big_billion") + " " + st;
                    }
                }
                else if (i == 5)
                {
                    if (N > 0)
                    {
                        st = StrV(N, ReManager, Lang) + " " + ReManager.GetString("big_trillon") + " " + st;
                    }
                }
            }
        ...
        {
            string txt = "";
            Thread.CurrentThread.CurrentUICulture = new System.Globalization.CultureInfo("mn-MN");
            ResourceManager rm = new ResourceManager("LeXPro.cur_text", Assembly.GetExecutingAssembly());
            txt = GetStrValue(month, rm, "mn-MN");
            return txt;
        }
        public static string NumberToText(string curValue, string strTUG, string LangPra)
        {
            try
            {
                string _Tug = "";
                string _Mun = "";
                Thread.CurrentThread.CurrentUICulture = new System.Globalization.CultureInfo(LangPra);
                ResourceManager rm = new ResourceManager("LeXPro.cur_text", Assembly.GetExecutingAssembly());
                try
                {
                    _Tug = rm.GetString("Tug_" + strTUG.ToUpper());
                }
                catch (Exception ex)
                {
                    _Tug = strTUG;
                ...
                try
                {
                    _Mun = rm.GetString("Mun_" + strTUG.ToUpper());
                }
                catch (Exception ex)
                {
                    _Mun = rm.GetString("fraction");
                }

                return NumberToText(curValue, _Tug, _Mun, LangPra, rm).ToLower();
            }
            catch (Exception ex)
            {
            }
        ...
        private static string NumberToText(string curValue, string strTUG, string strMUN, string LangPram, ResourceManager ReMan)
        {
            string strReturn;
            ...
            string[] strTmp = curValue.Split('.');

            // Бүхэл хэсгийг текст болгох
            strReturn = GetStrValue(strTmp[0], ReMan, LangPram);

            if (strReturn.Trim().Length == 0)
                strReturn = "" + ReMan.GetString("unit_0") + " " + strTUG;
            else
                strReturn = strReturn + " " + strTUG;

            //Бутархай хэсгийг текст болгох
            if (strTmp.Length > 1 && Convert.ToInt32(strTmp[1]) > 0)
            {
                if (strTUG.Length > 1)
                ...
                try
                {
                    decimalSep = ReMan.GetString("decimal_sep");
                }
                catch { }

                strReturn += decimalSep + GetStrValue(strTmp[1], ReMan, LangPram) + " " + strMUN;
            }

            if (boolNegative)
            {
                strReturn = "" + ReMan.GetString("minus") + "" + strReturn;
            }
            return strReturn.Trim();
        }

        private static string StrV11(string S, ResourceManager ReMan)
        {
            ...
            {
                return ReMan.GetString("stp_" + S.Substring(0, 1));
            }
        }
        private static string StrV2(string S, ResourceManager ReMan, string Lang, string XValue, ref bool Aravt)
        {
            if (S.Substring(0, 1) == "1")
            {
                Aravt = true;
                return ReMan.GetString("eng_" + XValue.Substring(1, 2));
            }
            else
            {
                Aravt = false;
                return ReMan.GetString("denary_" + S.Substring(0, 1));
            }
            if (S.Substring(0, 1) == "0")
            {
                Aravt = false;
        ...
        private static string StrV(int S, ResourceManager ReMan, string Lang)
        {
            string SS;
            string c;
            string XValue;
            bool Aravt = false;
            //X = "000" + S.ToString();111
            //X = Right(X, 3);
            XValue = S.ToString().PadLeft(3, '0');//.Substring(X.Length - 3, 3);
            SS = "";
            c = XValue.Substring(0, 1);

            if (Convert.ToInt32(c) > 0)
            {
                if (Convert.ToInt32(c) == 1)
                {
                    SS = SS + " " + ReMan.GetString("one_hundred");
                }
                else
                {
                    SS = SS + " " + StrV11(c, ReMan) + " " + ReMan.GetString("big_hundred");
                }
            }
            c = XValue.Substring(1, 1);
            if (Convert.ToInt32(c) > 0)
            {
                SS = SS + " " + StrV2(c, ReMan, Lang, XValue, ref Aravt);
            }
            if (!Aravt)
            {
                c = XValue.Substring(2, 1);
                if (Convert.ToInt32(c) > 0)
                {
                    if (Convert.ToInt32(c) == 1)
                    {
                        if (Convert.ToInt32(XValue.Substring(1, 1)) == 0)
                        {
                            SS = SS + " " + ReMan.GetString("unit_1");
                        }
                        else
                        {
                            SS = SS + " " + StrV11(c, ReMan);
                        }
                    }
                    else
                    {
